...
export async function getProfileById(profileId: string): Promise<{ success: boolean; data?: FullProfile; error?: string }> {
  try {
    ...
    const formattedData: FullProfile = {
      ...
      last_name: isPremium ? (profile.last_name || "") : "",
      age,
      ...
      bio: isPremium ? profile.bio : null,
      verification_status: profile.verification_status || "unverified",
      education: isPremium ? profile.education : null,
      university: isPremium ? profile.university : null,
      company: isPremium ? profile.company : null,
      income_range: isPremium ? profile.income_range : null,
      phone: isPremium ? profile.phone : null,
      instagram: isPremium ? profile.instagram : null,
      compatibility_score: Math.floor(Math.random() * 15) + 80, // Mock compatibility logic
      images: isPremium ? images : images.slice(0, 2), // Send at most 2 images to free users to prevent leaking gallery
